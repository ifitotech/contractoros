...
type SupplyRequest = { id: string; supplier: string; project: string; items: number; partNumber: string; description: string; unit: string; notes?: string; status: string; date: string };
const initialRequests: SupplyRequest[] = [
  { id: "SR-001", supplier: "Graybar Electric", project: "Casa Rivera", items: 8, partNumber: "QO120GFI", description: "20A GFCI breaker", unit: "each", status: "Waiting for pricing", date: "Jul 28, 2026" },
  { id: "SR-002", supplier: "Ferguson", project: "Office Torres", items: 4, partNumber: "LITH-2x4-40K", description: "2x4 LED panels", unit: "each", status: "Pricing received", date: "Jul 27, 2026" },
];
...
export default function SupplyRequestsPage() {
  ...
  return <div className="p-4 md:p-8 max-w-4xl mx-auto">
    ...
    {open && <div className="fixed inset-0 z-50 bg-slate-950/30 p-4 flex items-end sm:items-center justify-center"><form action={addRequest} className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl"><div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold">New Supply Request</h2><p className="text-xs text-slate-500 mt-1">Use a catalog part number or add a manual item.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close"><X className="w-5 h-5" /></button></div><div className="space-y-3"><input name="supplier" required placeholder="Supplier name" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /><input name="project" required placeholder="Project or client" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /><input name="partNumber" placeholder="Part number (optional for manual item)" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /><input name="description" required placeholder="Item description" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /><div className="grid grid-cols-2 gap-3"><input name="items" type="number" min="1" defaultValue="1" required placeholder="Quantity" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /><select name="unit" defaultValue="each" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="each">Each</option><option value="ft">ft</option><option value="box">Box</option><option value="roll">Roll</option><option value="lot">Lot</option></select></div><textarea name="notes" rows={2} placeholder="Notes (optional)" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm resize-none" /></div><Button type="submit" className="w-full mt-5">Create request</Button></form></div>}
  </div>;
}
