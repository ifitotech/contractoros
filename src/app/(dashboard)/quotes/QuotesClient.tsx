...
export default function QuotesClient({ quotes, demo = false }: { quotes?: Quote[]; demo?: boolean }) {
  ...
  return <div className="p-4 md:p-8">
    ...
    <div className="mb-6 grid gap-4 lg:grid-cols-2">
      <div className="surface-card p-4 md:p-5"><div className="mb-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-600">Customer work</p><h2 className="mt-1 text-lg font-bold">Customer quotes</h2><p className="mt-1 text-xs text-slate-500">Quotes your company sends to clients for approval.</p></div><div className="grid grid-cols-2 gap-2"><ModeLink href="/quotes/new?type=service" label={t("createServiceQuote")} icon={<Wrench className="h-4 w-4" />} /><ModeLink href="/quotes/new?type=complete" label={t("createCompleteQuote")} icon={<FileText className="h-4 w-4" />} /><ModeLink href="/quotes/new?type=materials" label={t("createMaterialQuote")} icon={<Layers className="h-4 w-4" />} /><ModeLink href="/quotes/estimator" label={t("createPlanEstimator")} icon={<Ruler className="h-4 w-4" />} /></div></div>
      <div className="surface-card border-amber-200 bg-amber-50/40 p-4 md:p-5"><div className="mb-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-700">Supply & purchasing</p><h2 className="mt-1 text-lg font-bold">Materials for your jobs</h2><p className="mt-1 text-xs text-slate-600">Request supplier pricing or prepare a purchase order. These are not customer quotes.</p></div><div className="grid grid-cols-2 gap-2"><ModeLink href="/supply-requests" label={t("createSupplyRequest")} icon={<ShoppingCart className="h-4 w-4" />} tone="amber" /><ModeLink href="/quotes/estimator" label="Plan estimator" icon={<Ruler className="h-4 w-4" />} tone="amber" /><ModeLink href="/pos/new" label="New purchase order" icon={<ClipboardList className="h-4 w-4" />} tone="amber" /><ModeLink href="/pos" label="View purchase orders" icon={<ArrowUpRight className="h-4 w-4" />} tone="amber" /></div></div>
    </div>
    <div className="mb-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Customer work</p><h2 className="mt-1 text-lg font-bold">Quotes pipeline</h2></div>
