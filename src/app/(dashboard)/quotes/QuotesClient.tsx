...
export default function QuotesClient({ quotes, demo = false }: { quotes?: Quote[]; demo?: boolean }) {
  ...
  return <div className="quotes-supply-page p-4 md:p-8">
    ...
    <div className="mb-6 grid gap-4 lg:grid-cols-2">
      <div className="surface-card border-amber-200 bg-amber-50/40 p-4 md:p-5"><div className="mb-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-700">Supply & purchasing</p><h2 className="mt-1 text-lg font-bold">Supply → Purchase</h2><p className="mt-1 text-xs text-slate-600">Build the material list, request supplier pricing, then create the purchase order.</p></div><div className="mb-3 flex items-center gap-2 text-[11px] font-semibold text-amber-800"><span className="rounded-full bg-amber-200 px-2 py-1">1 Request</span><span className="h-px flex-1 bg-amber-200" /><span className="rounded-full bg-amber-200 px-2 py-1">2 Review</span><span className="h-px flex-1 bg-amber-200" /><span className="rounded-full bg-amber-200 px-2 py-1">3 Buy</span></div><div className="grid grid-cols-2 gap-2"><ModeLink href="/supply-requests" label={t("createSupplyRequest")} icon={<ShoppingCart className="h-4 w-4" />} tone="amber" /><ModeLink href="/quotes/new?type=materials" label={t("createMaterialQuote")} icon={<Layers className="h-4 w-4" />} tone="amber" /><ModeLink href="/quotes/new?type=complete" label={t("createCompleteQuote")} icon={<FileText className="h-4 w-4" />} tone="amber" /><ModeLink href="/quotes/estimator" label="Plan estimator" icon={<Ruler className="h-4 w-4" />} tone="amber" /><ModeLink href="/pos/new" label="New purchase order" icon={<ClipboardList className="h-4 w-4" />} tone="amber" /><ModeLink href="/pos" label="View purchase orders" icon={<ArrowUpRight className="h-4 w-4" />} tone="amber" /></div></div>
    </div>
